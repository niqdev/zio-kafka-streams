      ...
      testM("topology") {
        for {
          result <- ZTestTopology.driver.use { driver =>
            for {
              source     <- driver.sourceAvro[DummyKey, DummyValue]("source-topic")
              sink       <- driver.sinkAvro[DummyKey, DummyValue]("sink-topic")
              _          <- source.produceValue(DummyValue("myValue"))
              dummyValue <- sink.consumeValue
            } yield dummyValue
          }
        } yield assert(result.value)(equalTo("MYVALUE"))
      }.provideSomeLayerShared((ZTestTopology.testConfigLayer >+> topologyLayer).mapError(TestFailure.fail))
    )
}
